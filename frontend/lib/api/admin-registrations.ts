...
export async function getAdminRegistrations(
  ...
  try {
    response = await fetch(
      `${baseUrl}/api/admin/registrations?limit=500&status=${status}`,
      {
        method: "GET",
        headers: {
...
export async function updateRegistrationTreated(
  ...
  try {
    response = await fetch(
      `${baseUrl}/api/admin/registrations/${id}/treated`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const payload = (await response.json()) as UpdateTreatedResponse;
